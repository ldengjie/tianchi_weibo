#library(xts)
#load("RDataTest")
#t7=t[as.Date(t$time)<as.Date("2014-08-01"),]
#t7<-read.csv("t7.txt",sep="\t",quote="")
...
#tn=t[((as.Date(t$time)>as.Date(beginDate))+(as.Date(t$time)<=as.Date(endDate)))==2,]
#tc=aggregate(tn,by=list(tn$time),FUN=funl[[2]])
...
tt=aggregate(tWithType,by=list(tWithType$time,tWithType$userType),FUN=funl[[1]])
#
yvf=NULL
yvc=NULL
yvl=NULL
