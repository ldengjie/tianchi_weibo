...
for(fi in 9:9)
{
    ...
    }else if(fi==9)
    {
        ...
        #for(tpi in 72)
        {
            ...
            #cat(">> tdm ")
            #print(Sys.time())
            ctl=list(removePunctuation=T,minDocFreq=1,wordLengths = c(1, Inf))
            #ctl=list(removePunctuation=T,minDocFreq=1,wordLengths = c(1, Inf),weighting = weightTfIdf)
            ...
            #tc=merge(tc,t[c(2,4,5,6)],by=c("mid"))
...
#
    ...
    if(fi==9) system("sed -e 's#^\"[0-9]*\",##g' -e 's#\",#	#g' -e 's#\"##g' -e '/_/d' r.txt > 9/weibo_result.txt")
    ##print(Sys.time())
